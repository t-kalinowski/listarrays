...
```
# listarrays
A toolbox for working with R arrays in a functional programming style. It offers
a family of functions for flexibly splitting, binding, modifying, and subsetting
arrays, as well as a handful of lower-level helpers that abstract out patterns
commonly encountered while working with arrays. 
...
+  `modify_along_dim()` takes an array, calls the passed function `.f()` on each
    ...
    (think of this as a safe alternative to `base::apply()` that is guaranteed 
    to return an array of the same shape as it received)
    
+  `extract_dim()` a wrapper around `[` that allows you to specify the dimension 
   being subset as an additional argument. For example, `extract_dim(X, 1, idx)`
   will extract on the first dimension, regardless how many dimensions are in 
...
*  A handful of lower-level helpers that abstract out patterns commonly
   ...
   combination of `seq_along()` and `nrow()`).
